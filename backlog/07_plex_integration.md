...
## Overview
Implement Plex API client for fetching playlist tracks and library artists. Since no official Go SDK exists, this requires direct HTTP API integration with XML parsing.
...
## Dependencies
- Previous: `06_caching_layer.md` (Cache service)
- New imports: `encoding/xml`, `net/url`
- Models: `gocommender/internal/models`
- Environment: `PLEX_URL`, `PLEX_TOKEN`

## Next Steps
Proceed to `08_llm_client.md` to implement OpenAI integration for artist recommendations.

## Notes
- Plex API uses XML format, requiring custom parsing
- No official Go SDK available - direct HTTP API integration
- Authentication via X-Plex-Token parameter in URLs
- Music library section key discovery needed for artist queries
- Rating scale is 1-10 in Plex (different from star ratings)
- Playlist types: regular vs smart playlists have different behaviors
- Error handling includes specific Plex API status codes
- Track metadata includes play counts and last played timestamps
- Genre filtering available but may have limited accuracy
- Server connection testing validates token and accessibility